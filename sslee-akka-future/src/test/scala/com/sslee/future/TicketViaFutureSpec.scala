...
class TicketViaFutureSpec extends WordSpecLike with MustMatchers {
  
  object TicketServiceImpl extends TicketService with MockTicketService
...
trait MockTicketService extends TicketService {
  import com.github.nscala_time.time.Imports._
  
  def getEvent(ticketNr: String, location: Location): Future[TicketInfo] = {
    Future {
      if(ticketNr == "1234")
        TicketInfo(ticketNr,location,event = Some(
            Event("Quasimoto", Location(4.324218908d,53.12311144d), new DateTime(2013,10,1,22,30))
        ))
  ...
  def callWeatherServiceY(ticketInfo: TicketInfo): Future[Option[Weather]] = {
    Future {
      Some(Weather(25,false))
    }
  }
  
  def callTraficService(origin: Location, destination: Location, time: DateTime): Future[Option[RouteByCar]] = {
    Future {
  ...
  def callSimilarArtistsService(event: Event): Future[Seq[Artist]] = {
    Future {
      //Seq(Artist("Metallica", "metallica.com/calendar"), Artist("Ozzy", "ozzy.com/calendar"), Artist("Bonjobi", "bonjobi/calendar"))
